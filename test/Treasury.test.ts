...
const INITIAL_YFLUSD_AMOUNT = utils.parseEther('50000');
const INITIAL_SYFL_AMOUNT = utils.parseEther('10000');
const INITIAL_BYFL_AMOUNT = utils.parseEther('5');
...
describe('Treasury', () => {
  ...
  beforeEach('deploy contracts', async () => {
    ...
    treasury = await Treasury.connect(operator).deploy(
      ...
      oracle.address,
      oracle.address,
      mockLinkswapPriceOracle.address,
      boardroom.address,
      fund.address,
      curve.address,
      startTime
  ...
  describe('seigniorage', () => {
    describe('#allocateSeigniorage', () => {
      beforeEach('transfer permissions', async () => {
        await bond.mint(operator.address, INITIAL_BYFL_AMOUNT);
        await cash.mint(operator.address, INITIAL_YFLUSD_AMOUNT);
        await cash.mint(treasury.address, INITIAL_YFLUSD_AMOUNT);
        await share.mint(operator.address, INITIAL_SYFL_AMOUNT);
    ...
    describe('after startTime', () => {
      ...
      describe('#redeemBonds', () => {
        beforeEach('allocate seigniorage to treasury', async () => {
          await cash.mint(operator.address, INITIAL_YFLUSD_AMOUNT);
          await bond.mint(operator.address, INITIAL_YFLUSD_AMOUNT);
          for await (const contract of [cash, bond, share, boardroom]) {
            await contract.connect(operator).transferOperator(treasury.address);
          }
